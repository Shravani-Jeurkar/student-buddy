<?php
    ...
    if(isset($_POST['add_room']))
    {
        ...
        $q1 = "INSERT INTO `rooms` (`name`, `area`, `price`, `quantity`, `adult`, `children`, `description`, `admin_id`) VALUES (?,?,?,?,?,?,?,?)";
        $values = [$frm_data['name'],$frm_data['area'],$frm_data['price'],$frm_data['quantity'],$frm_data['adult'],$frm_data['children'],$frm_data['desc'], $_SESSION['adminID']];
    ...
    {
        $res = select('SELECT * FROM `rooms` Where `admin_id`=? AND `removed`=?',[$_SESSION['adminID'] ,0], 'ii');
        $i = 1;
